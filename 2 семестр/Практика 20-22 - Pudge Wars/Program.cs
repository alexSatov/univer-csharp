...
namespace PudgeClient
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            { 
                if(i < path.Count)
                {
                    ...
                    pudge.Direction = nextDirection;
                    sensorData = client.Rotate(rotationAngle);
                    while (SegmentLength(pudge.Location, path[i + 1].Position) > 7.8)
                    {
                        sensorData = client.Move(8);
                        pudge.Location = new Point((int)sensorData.SelfLocation.X, (int)sensorData.SelfLocation.Y);
                    }
        ...
        public static void PrintPath(List<Node> path)
        {
            ...
            {
                Console.Write(string.Format("({0}, {1}), ", node.Position.X, node.Position.Y));
            }
        }
        ...
        {
            return new List<Node>() {  new Node(new Point(130, 130), "empty"),
                                       new Node(new Point(-130, -130), "rune")
                                    };
        }
    }
